...
_InstInfo* inst_lookup(_CodeInfo* ci, _PrefixState* ps)
{
    ...
    if (tmpIndex0 == INST_WAIT_INDEX) {
        ...
        case 0xDF:
            break;
        /* WAIT is not a prefix! */
        default: return NULL; /* Faster to return NULL, it will be detected as WAIT later anyway. */
        }
    }
    ...
    if ((instType == INT_LIST_GROUP) && (!isWaitIncluded)) {
        return inst_get_info(in, (tmpIndex1 >> 3) & 7);
    }
