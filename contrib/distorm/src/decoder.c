...
static _DecodeResult decode_inst(_CodeInfo* ci, _PrefixState* ps, _DInst* di)
{
     ...
     */
    if (isi->d != OT_NONE) {
        startInst = startCode - ps->last + ps->start;
        if (!operands_extract(ci, di, startInst, ii, instFlags, (_OpType)isi->d, ONT_1, modrm, ps, effOpSz, effAdrSz, &lockable))
            goto _Undecodable;
    } else  goto _OperandsExtracted;

    if (isi->s != OT_NONE) {
        if (!operands_extract(ci, di, startInst, ii, instFlags, (_OpType)isi->s, ONT_2, modrm, ps, effOpSz, effAdrSz, NULL))
            goto _Undecodable;
    } else  goto _OperandsExtracted;

    /* Use third operand, only if the flags says this InstInfo requires it. */
    if (instFlags & INST_USE_OP3) {
        if (!operands_extract(ci, di, startInst, ii, instFlags, (_OpType)((_InstInfoEx*)ii)->op3, ONT_3, modrm, ps, effOpSz, effAdrSz, NULL))
            goto _Undecodable;
    } else  goto _OperandsExtracted;

    /* Support for a fourth operand is added for (i.e:) INSERTQ instruction. */
    if (instFlags & INST_USE_OP4) {
        if (!operands_extract(ci, di, startInst, ii, instFlags, (_OpType)((_InstInfoEx*)ii)->op4, ONT_4, modrm, ps, effOpSz, effAdrSz, NULL))
            goto _Undecodable;
    }
_OperandsExtracted:
     ...
     */
    if ((ci->code - ps->start) > INST_MAXIMUM_SIZE)
        goto _Undecodable; /* Drop instruction. */
    ...
    */
         ...
         */
        if ((instFlags & INST_USE_EXMNEMONIC) && ((instFlags & (INST_32BITS | INST_64BITS)) == 0))
        ...
        if (instFlags & (INST_USE_EXMNEMONIC | INST_USE_EXMNEMONIC2)) {
            ...
            if ((instFlags & INST_USE_EXMNEMONIC2) && (vrex & PREFIX_EX_W)) {
                ...
                di->opcode = ((_InstInfoEx*)ii)->opcodeId2; /* Use second mnemonic. */
            }
        } else {
            di->opcode = ii->opcodeId;
        }
    ...
    if ((instFlags & INST_NATIVE) && (ps->decodedPrefixes & INST_PRE_OP_SIZE))
        ps->usedPrefixes |= INST_PRE_OP_SIZE;
    ...
    /* Copy DST_WR flag. */
    if (instFlags & INST_DST_WR)
        di->flags |= FLAG_DST_WR;
...
_DecodeResult decode_internal(_CodeInfo* _ci, int supportOldIntr, _DInst result[], unsigned int maxResultCount, unsigned int* usedInstructionsCount)
{
     ...
     */
    if (_ci->features & DF_MAXIMUM_ADDR32)
        addrMask = 0xffffffff;
    else if (_ci->features & DF_MAXIMUM_ADDR16)
        addrMask = 0xffff;
#endif
    ...
    *usedInstructionsCount = 0;
    ...
    while (codeLen > 0) {
        ...
        if (prefixes_is_valid(*code, ci.dt)) {
            ...
            if (prefixSize == INST_MAXIMUM_SIZE)
                continue;
        }
        ...
        if ((_ci->features & DF_RETURN_FC_ONLY) && (META_GET_FC(pdi->meta) == FC_NONE))
            decodeResult = DECRES_FILTERED;
