...
#define OP_SIZE8_BITS       8
#define OP_SIZE16_BITS      16
#define OP_SIZE32_BITS      32
#define OP_SIZE64_BITS      64
#define OP_SIZE80_BITS      80
#define OP_SIZE128_BITS     128
#define OP_SIZE256_BITS     256
#define OP_SIZE8_BYTES      (OP_SIZE8_BITS >> 3)
#define OP_SIZE16_BYTES     (OP_SIZE16_BITS >> 3)
#define OP_SIZE32_BYTES     (OP_SIZE32_BITS >> 3)
#define OP_SIZE64_BYTES     (OP_SIZE64_BITS >> 3)
#define OP_SIZE80_BYTES     (OP_SIZE80_BITS >> 3)
#define OP_SIZE128_BYTES    (OP_SIZE128_BITS >> 3)
#define OP_SIZE256_BYTES    (OP_SIZE256_BITS >> 3)
...
static uint8_t _OPSIZETOINT[] = {0, OP_SIZE8_BYTES, OP_SIZE16_BYTES, OP_SIZE32_BYTES, OP_SIZE64_BYTES, OP_SIZE80_BYTES, OP_SIZE128_BYTES, OP_SIZE256_BYTES};
...
{
    if ((reg >= 4) && (reg < 8))
        return reg + REGS8_REX_BASE - 4;
    return reg + REGS8_BASE;
}

/* A helper function to set operand's type and size. */
static void _FASTCALL_ operands_set_ts(_Operand* op, _OperandType type, uint8_t size)
{
    op->type = type;
    op->size = size;
}
...
static void _FASTCALL_ operands_set_tsi(_Operand* op, _OperandType type, uint8_t size, unsigned int index)
{
    op->type = type;
...
static int _FASTCALL_ read_stream_safe_uint(_CodeInfo* ci, void* result, unsigned int size)
{
    ...
    switch (size) {
    case OP_SIZE8_BYTES: *(uint8_t*)result = *(uint8_t*)ci->code; break;
    case OP_SIZE16_BYTES: *(uint16_t*)result = RUSHORT(ci->code); break;
    case OP_SIZE32_BYTES: *(uint32_t*)result = RULONG(ci->code); break;
    case OP_SIZE64_BYTES: *(uint64_t*)result = RULLONG(ci->code); break;
    }
...
static int _FASTCALL_ read_stream_safe_sint(_CodeInfo* ci, int64_t* result, unsigned int size)
{
    ...
    switch (size) {
    case OP_SIZE8_BYTES: *result = *(int8_t*)ci->code; break;
    case OP_SIZE16_BYTES: *result = RSHORT(ci->code); break;
    case OP_SIZE32_BYTES: *result = RLONG(ci->code); break;
    case OP_SIZE64_BYTES: *result = RLLONG(ci->code); break;
    }
                                 ...
                                 unsigned int sib, unsigned int mod)
{
    ...
    if (base != 5) {
        if (vrex & PREFIX_EX_B)
            ps->usedPrefixes |= INST_PRE_REX;
        *pIndex = (uint8_t)(effAdrSz == Decode64Bits ? REGS64_BASE : REGS32_BASE);
        *pIndex += (uint8_t)(base + ((vrex & PREFIX_EX_B) ? EX_GPR_BASE : 0));
        ...
        if (effAdrSz == Decode64Bits) {
            *pIndex = (uint8_t)(REGS64_BASE + 5 + ((vrex & PREFIX_EX_B) ? EX_GPR_BASE : 0));
        } else {
            *pIndex = (uint8_t)(REGS32_BASE + 5 + ((vrex & PREFIX_EX_B) ? EX_GPR_BASE : 0));
        }
    } else if (index == 4) {
    ...
    if (index != 4) { /* In 64 bits decoding mode, if index == R12, it's valid! */
        if (effAdrSz == Decode64Bits) {
            op->index = (uint8_t)(REGS64_BASE + index);
        } else {
            op->index = (uint8_t)(REGS32_BASE + index);
        }
        di->scale = (uint8_t)(scale != 0 ? (1 << scale) : 0);
    }
}
                                  ...
                                  _iflags instFlags)
{
    ...
    if (mod == 3) {
        ...
        op->size = size;
        op->index = (uint8_t)rm;
        op->offset = (uint8_t)(ci->code - startInst); /* Set the offset of this operand in the instruction. */
        return TRUE;
    }
    ...
    if (lockableInstruction && (ps->decodedPrefixes & INST_PRE_LOCK))
    ...
    if (effAdrSz == Decode16Bits) {
        ...
        if ((mod == 0) && (rm == 6)) {
            ...
            op->type = O_DISP;
            op->offset = (uint8_t)(ci->code - startInst); /* Set the disp offset in the instruction. */
            di->dispSize = OP_SIZE16_BYTES;
            di->dispOffset = op->offset;
            if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, OP_SIZE16_BYTES))
                return FALSE;
        } else {
            /*
            ...
            if (mod == 1) { /* 8 bits displacement + indirection */
                di->dispSize = OP_SIZE8_BYTES;
                di->dispOffset = op->offset;
                if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, OP_SIZE8_BYTES))
                    return FALSE;
        ...
        if ((mod == 0) && (rm == 5)) {
            ...
            op->offset = (uint8_t)(ci->code - startInst); /* Set the disp offset in the instruction. */
            di->dispSize = OP_SIZE32_BYTES;
            di->dispOffset = op->offset;
            if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, OP_SIZE32_BYTES))
                return FALSE;
            ...
            if (rm == 4) {
                ...
                /* Read SIB byte. */
                if (!read_stream_safe_uint(ci, &sib, OP_SIZE8_BYTES))
                    return FALSE;
                ...
                if (effAdrSz == Decode64Bits) {
                    op->index = (uint8_t)(REGS64_BASE + rm);
                } else {
                    op->index = (uint8_t)(REGS32_BASE + rm);
                }
            }
        ...
        if (di->base != R_NONE) {
            ...
            base = 0; /* If it's only an index but got scale, it's still DS. */
        }
...
int operands_extract(_CodeInfo* ci, _DInst* di, const uint8_t* startInst, _InstInfo* ii,
...
{
    int ret;
    unsigned int mod, reg, rm, vexV = ps->vexV;
    ...
    case OT_MEM_OPT:
        ...
        if (mod == 0x3) {
            /* Set the operand offset in the instruction. */
            op->offset = (uint8_t)(ci->code - startInst);
            return TRUE;
        }
    break;
    ...
    case OT_LMEM128_256:
        if (vrex & PREFIX_EX_L) {
            opSize = OPERAND_SIZE256;
        } else {
            opSize = OPERAND_SIZE128;
        }
    break;
    case OT_MEM:
        /* Size is unknown, but still handled. */
    break;
    ...
    if (typeHandled) {
        ...
        if (mod == 0x3) {
            if (lockableInstruction)
                *lockableInstruction = FALSE;
            return FALSE;
        }
        op->size = _OPSIZETOINT[opSize];
        ret = operands_extract_modrm(ci, di, startInst, type, opNum, ps, effOpSz, effAdrSz, lockableInstruction, mod, rm, instFlags);
    ...
    switch (type) {
    case OT_IMM8:
        operands_set_ts(op, O_IMM, OP_SIZE8_BYTES);
        op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
        if (!read_stream_safe_uint(ci, &di->imm.byte, OP_SIZE8_BYTES))
            return FALSE;
        break;
    ...
    case OT_IMM16: /* Force 16 bits imm. */
            operands_set_ts(op, O_IMM, OP_SIZE16_BYTES);
            op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
            if (!read_stream_safe_uint(ci, &di->imm.word, OP_SIZE16_BYTES))
                return FALSE;
            break;
        /*
         ...
         */
        } else if ((effOpSz == Decode64Bits) && ((instFlags & (INST_64BITS | INST_PRE_REX)) == (INST_64BITS | INST_PRE_REX))) {
            ps->usedPrefixes |= INST_PRE_REX;

            operands_set_ts(op, O_IMM, OP_SIZE64_BYTES);
            op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
            if (!read_stream_safe_uint(ci, &di->imm.qword, OP_SIZE64_BYTES))
                return FALSE;
            break;
        } else {
            ps->usedPrefixes |= INST_PRE_OP_SIZE;
    ...
    case OT_IMM32:
        ...
        if (ci->dt == Decode64Bits) {
             ...
             */
            op->size = OP_SIZE32_BYTES;
            /* Use this as an indicator that it should be signed extended. */
            di->flags |= FLAG_IMM_SIGNED;
            op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
            if (!read_stream_safe_sint(ci, &di->imm.sqword, OP_SIZE32_BYTES))
                return FALSE;
        } else {
            op->size = OP_SIZE32_BYTES;
            op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
            if (!read_stream_safe_uint(ci, &di->imm.dword, OP_SIZE32_BYTES))
                return FALSE;
        }
        break;
    case OT_SEIMM8: /* Sign extended immediate. */
        ...
        if ((instFlags & INST_PRE_OP_SIZE) && (ps->decodedPrefixes & INST_PRE_OP_SIZE)) {
            ...
            case Decode32Bits:
            case Decode64Bits:
                op->size = OP_SIZE16_BYTES;
            break;
            }
        } else {
            op->size = OP_SIZE8_BYTES;
        }
        di->flags |= FLAG_IMM_SIGNED;
        op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
        if (!read_stream_safe_sint(ci, &di->imm.sqword, OP_SIZE8_BYTES))
            return FALSE;
        break;
    case OT_IMM16_1:
        operands_set_ts(op, O_IMM1, OP_SIZE16_BYTES);
        op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
        if (!read_stream_safe_uint(ci, &di->imm.ex.i1, OP_SIZE16_BYTES))
            return FALSE;
        break;
    case OT_IMM8_1:
        operands_set_ts(op, O_IMM1, OP_SIZE8_BYTES);
        op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
        if (!read_stream_safe_uint(ci, &di->imm.ex.i1, OP_SIZE8_BYTES))
            return FALSE;
        break;
    case OT_IMM8_2:
        operands_set_ts(op, O_IMM2, OP_SIZE8_BYTES);
        op->offset = (uint8_t)(ci->code - startInst); /* Set the imm offset in the instruction. */
        if (!read_stream_safe_uint(ci, &di->imm.ex.i2, OP_SIZE8_BYTES))
            return FALSE;
        break;
    case OT_REG8:
        operands_set_ts(op, O_REG, OP_SIZE8_BYTES);
        if (ps->prefixExtType) {
            /*
            ...
            op->index = (uint8_t)(REGS8_BASE + reg);
        }
        break;
    case OT_REG16:
        operands_set_tsi(op, O_REG, OP_SIZE16_BYTES, REGS16_BASE + reg);
        break;
    case OT_REG_FULL:
        switch (effOpSz) {
        case Decode16Bits:
        ...
        case Decode32Bits:
            ...
            operands_set_tsi(op, O_REG, OP_SIZE32_BYTES, REGS32_BASE + reg);
            break;
        case Decode64Bits: /* rex must be presented. */
            ps->usedPrefixes |= INST_PRE_REX;
            operands_set_tsi(op, O_REG, OP_SIZE64_BYTES, REGS64_BASE + reg + ((vrex & PREFIX_EX_R) ? EX_GPR_BASE : 0));
            break;
        }
        break;
    case OT_REG32:
    ...
    case OT_REG32_64: /* Handle CVT's, MOVxX and MOVNTI instructions which could be extended to 64 bits registers with REX. */
        ...
        /* Is it a promoted instruction? (only INST_64BITS is set and REX isn't required.) */
        if ((ci->dt == Decode64Bits) && ((instFlags & (INST_64BITS | INST_PRE_REX)) == INST_64BITS)) {
            operands_set_tsi(op, O_REG, OP_SIZE64_BYTES, REGS64_BASE + reg);
            op->offset = (uint8_t)(ci->code - startInst); /* Set the reg offset in the instruction. */
            break;
        }
        /* Give a chance to REX.W. Because if it was a promoted instruction we don't care about REX.W anyways. */
        if (vrex & PREFIX_EX_W) {
    ...
    case OT_FREG32_64_RM: /* Force decoding mode. Used for MOV CR(n)/DR(n) which defaults to 64 bits operand size in 64 bits. */
        ...
        if (ci->dt == Decode64Bits) {
            operands_set_tsi(op, O_REG, OP_SIZE64_BYTES, REGS64_BASE + rm);
        } else {
            operands_set_tsi(op, O_REG, OP_SIZE32_BYTES, REGS32_BASE + rm);
        }
        break;
    case OT_MM: /* MMX register */
        operands_set_tsi(op, O_REG, OP_SIZE64_BYTES, MMXREGS_BASE + reg);
        break;
    case OT_MM_RM: /* MMX register, this time from the RM field */
        operands_set_tsi(op, O_REG, OP_SIZE64_BYTES, MMXREGS_BASE + rm);
        break;
    ...
    case OT_XMM: /* SSE register */
        ...
        operands_set_tsi(op, O_REG, OP_SIZE128_BYTES, SSEREGS_BASE + reg);
        break;
    case OT_XMM_RM: /* SSE register, this time from the RM field */
        ...
        operands_set_tsi(op, O_REG, OP_SIZE128_BYTES, SSEREGS_BASE + rm);
        break;
    case OT_CREG:
        /*
        ...
        if ((reg >= CREGS_MAX) || (reg == 1) || ((reg >= 5) && (reg <= 7)))
            return FALSE;

        op->type = O_REG;
        if (ci->dt == Decode64Bits) {
            op->size = OP_SIZE64_BYTES;
        } else {
            op->size = OP_SIZE32_BYTES;
        }
    ...
    case OT_DREG:
         ...
         */
        if ((reg == 4) || (reg == 5) || (vrex & PREFIX_EX_R))
            return FALSE;
    ...
    case OT_SREG: /* Works with REG16 only! */
        ...
        if (lockableInstruction && (reg == 1))
            return FALSE; /* Can't MOV CS, <REG>. */
        /*Don't parse if the reg exceeds the bounds of the array. */
        if (reg <= SEG_REGS_MAX - 1) {
            operands_set_tsi(op, O_REG, OP_SIZE16_BYTES, SREGS_BASE + reg);
            op->offset = (uint8_t)(ci->code - startInst); /* Set the reg offset in the instruction. */
        } else {
            return FALSE;
        }
    ...
    case OT_SEG:
        ...
        break;
    case OT_ACC8:
        operands_set_tsi(op, O_REG, OP_SIZE8_BYTES, R_AL);
        break;
    case OT_ACC16:
        operands_set_tsi(op, O_REG, OP_SIZE16_BYTES, R_AX);
        break;
    ...
    case OT_ACC_FULL:
        if (effOpSz == Decode16Bits) {
            ...
            ps->usedPrefixes |= INST_PRE_OP_SIZE;
            operands_set_tsi(op, O_REG, OP_SIZE32_BYTES, R_EAX);
        } else { /* Decode64Bits */
            ...
            if (~instFlags & INST_64BITS)
                ps->usedPrefixes |= INST_PRE_REX;
            operands_set_tsi(op, O_REG, OP_SIZE64_BYTES, R_RAX);
        }
        break;
    case OT_PTR16_FULL:
        ...
        if (effOpSz == Decode16Bits) {
            ...
            operands_set_ts(op, O_PTR, OP_SIZE16_BYTES);
            op->offset = (uint8_t)(ci->code - startInst); /* Set the ptr offset in the instruction. */
            ...
            operands_set_ts(op, O_PTR, OP_SIZE32_BYTES);
            op->offset = (uint8_t)(ci->code - startInst); /* Set the ptr offset in the instruction. */
    ...
    case OT_RELC_FULL:
        if (type == OT_RELCB) {
            operands_set_ts(op, O_PC, OP_SIZE8_BYTES);
            op->offset = (uint8_t)(ci->code - startInst); /* Set the rel offset in the instruction. */
            if (!read_stream_safe_sint(ci, &di->imm.sqword, OP_SIZE8_BYTES))
                return FALSE;
        } else { /* OT_RELC_FULL */
            ...
            op->offset = (uint8_t)(ci->code - startInst); /* Set the rel offset in the instruction. */
            if (effOpSz == Decode16Bits) {
                operands_set_ts(op, O_PC, OP_SIZE16_BYTES);
                if (!read_stream_safe_sint(ci, &di->imm.sqword, OP_SIZE16_BYTES))
                    return FALSE;
            } else { /* Decode32Bits or Decode64Bits = for now they are the same */
                operands_set_ts(op, O_PC, OP_SIZE32_BYTES);
                if (!read_stream_safe_sint(ci, &di->imm.sqword, OP_SIZE32_BYTES))
                    return FALSE;
            }
        }
    ...
    case OT_MOFFS_FULL:
        ...
        if (op->size == 0) {
            ...
            switch (effOpSz) {
            case Decode16Bits: op->size = OP_SIZE16_BYTES; break;
            case Decode32Bits: op->size = OP_SIZE32_BYTES; break;
            case Decode64Bits: op->size = OP_SIZE64_BYTES; break;
            }
        }
        ...
        if (effAdrSz == Decode16Bits) {
            ...
            di->dispSize = OP_SIZE16_BYTES;
            di->dispOffset = op->offset;
            if (!read_stream_safe_uint(ci, &di->disp, OP_SIZE16_BYTES))
                return FALSE;
            ...
            di->dispSize = OP_SIZE32_BYTES;
            di->dispOffset = op->offset;
            if (!read_stream_safe_uint(ci, &di->disp, OP_SIZE32_BYTES))
                return FALSE;
        } else { /* Decode64Bits */
            op->offset = (uint8_t)(ci->code - startInst); /* Set the disp offset in the instruction. */
            di->dispSize = OP_SIZE64_BYTES;
            di->dispOffset = op->offset;
            if (!read_stream_safe_uint(ci, &di->disp, OP_SIZE64_BYTES))
                return FALSE;
        }
        break;
    case OT_CONST1:
        ...
        break;
    case OT_REGCL:
        operands_set_tsi(op, O_REG, OP_SIZE8_BYTES, R_CL);
        break;

    case OT_FPU_SI:
        /* Low 3 bits specify the REG, similar to the MODR/M byte reg. */
        operands_set_tsi(op, O_REG, OP_SIZE32_BYTES, (unsigned int)(FPUREGS_BASE + (*(ci->code - 1) & 7)));
        op->offset = (uint8_t)((ci->code-1) - startInst); /* Set the reg offset in the instruction. */
        break;
    case OT_FPU_SSI:
        operands_set_tsi(op, O_REG, OP_SIZE32_BYTES, R_ST0);
        op->offset = (uint8_t)(ci->code - startInst); /* Set the reg offset in the instruction. */
        operands_set_tsi(op + 1, O_REG, OP_SIZE32_BYTES, (unsigned int)(FPUREGS_BASE + (*(ci->code - 1) & 7)));
        (op + 1)->offset = (uint8_t)((ci->code-1) - startInst); /* Set the reg offset in the instruction. */
        break;
    case OT_FPU_SIS:
        operands_set_tsi(op, O_REG, OP_SIZE32_BYTES, (unsigned int)(FPUREGS_BASE + (*(ci->code - 1) & 7)));
        op->offset = (uint8_t)((ci->code-1) - startInst); /* Set the reg offset in the instruction. */
        operands_set_tsi(op + 1, O_REG, OP_SIZE32_BYTES, R_ST0);
        (op + 1)->offset = (uint8_t)(ci->code - startInst); /* Set the reg offset in the instruction. */
        break;
    ...
    case OT_REGI_EDI:
        ...
        if ((opNum == ONT_1) && (ci->dt != Decode64Bits)) {
            di->segment = R_ES | SEGMENT_DEFAULT; /* No ES in 64 bits mode. */
        }
    ...
    case OT_REGDX:
        ...
        operands_set_tsi(op, O_REG, OP_SIZE16_BYTES, R_DX);
        break;

    /* Used for INVLPGA instruction. */
    case OT_REGECX:
        operands_set_tsi(op, O_REG, OP_SIZE32_BYTES, R_ECX);
        break;
    case OT_REGI_EBXAL:
        /* XLAT BYTE [rBX + AL] */
        ps->usedPrefixes |= INST_PRE_ADDR_SIZE;
        ...
        operands_set_tsi(op, O_MEM, OP_SIZE8_BYTES, R_AL);
    ...
    case OT_REGI_EAX:
         ...
         */
        if (effAdrSz == Decode64Bits) {
            operands_set_tsi(op, O_SMEM, OP_SIZE64_BYTES, R_RAX);
        } else if (effAdrSz == Decode32Bits) {
            ps->usedPrefixes |= INST_PRE_ADDR_SIZE;
            operands_set_tsi(op, O_SMEM, OP_SIZE32_BYTES, R_EAX);
        } else {
            ps->usedPrefixes |= INST_PRE_ADDR_SIZE;
            operands_set_tsi(op, O_SMEM, OP_SIZE16_BYTES, R_AX);
        }
        break;
    case OT_VXMM:
        operands_set_tsi(op, O_REG, OP_SIZE128_BYTES, SSEREGS_BASE + vexV);
        break;
    case OT_XMM_IMM:
        ci->codeLen -= sizeof(int8_t);
        if (ci->codeLen < 0)
            return FALSE;
        if (ci->dt == Decode32Bits) {
            reg = (unsigned int)((*ci->code >> 4) & 0x7);
        } else {
            reg = (unsigned int)((*ci->code >> 4) & 0xf);
        }
        operands_set_tsi(op, O_REG, OP_SIZE128_BYTES, SSEREGS_BASE + reg);
        op->offset = (uint8_t)(ci->code - startInst); /* Set the reg offset in the instruction. */
    ...
    case OT_YXMM:
        ...
        if (ps->vrex & PREFIX_EX_L) {
            operands_set_tsi(op, O_REG, OP_SIZE256_BYTES, AVXREGS_BASE + reg);
        } else {
            operands_set_tsi(op, O_REG, OP_SIZE128_BYTES, SSEREGS_BASE + reg);
        }
        break;
    ...
    case OT_YMM:
        ...
        break;
    case OT_VYMM:
        operands_set_tsi(op, O_REG, OP_SIZE256_BYTES, AVXREGS_BASE + vexV);
        break;
    case OT_VYXMM:
        if (ps->vrex & PREFIX_EX_L) {
            operands_set_tsi(op, O_REG, OP_SIZE256_BYTES, AVXREGS_BASE + vexV);
