...
#else
#define OFFSET_INTEGER unsigned long /* 32 bit offsets are used. */
#endif
...
typedef union {
    ...
    struct {
        uint16_t seg;
        /* Can be 16 or 32 bits, size in bytes is in ops[n].size. */
        uint32_t off;
    } ptr;
...
typedef struct {
    ...
    */
    ...
    /* Size in bytes of:
        ...
        O_PTR: size of instruction.imm.ptr.off (2 or 4)
    */
    uint8_t size;

    /* Offset in bytes into instruction of operand:
        O_REG: register
        O_IMM: instruction.imm
        O_IMM1: instruction.imm.ex.i1
        O_IMM2: instruction.imm.ex.i2
        O_DISP: instruction.disp
        O_SMEM: offset of indirection.
        O_MEM: offset of indirection.
        O_PC: offset of the relative offset
        O_PTR: offset of instruction.imm.ptr.off (2 or 4)
    */
    uint8_t offset;
...
/* See flag FLAG_GET_XXX macros below. */
 ...
 * 11 - reserved
 *
 * If you call these set-macros more than once, you will have to clean the bits
 * before doing so.
 */
...
/* To get the LOCK/REPNZ/REP/BND prefixes. */
...
#define INSTRUCTION_GET_TARGET(di)                                             \
    ((_OffsetType)(((di)->addr + (di)->imm.addr + (di)->size)))
/* Get the target address of a RIP-relative memory indirection. */
#define INSTRUCTION_GET_RIP_TARGET(di)                                         \
    ((_OffsetType)(((di)->addr + (di)->disp + (di)->size)))
...
#define SEGMENT_IS_DEFAULT(segment) \
    (((segment)&SEGMENT_DEFAULT) == SEGMENT_DEFAULT)
...
typedef struct {
    ...
    uint8_t segment; // 0x2F
    /* Used by ops[n].type == O_MEM. Base global register index (might be R_NONE), scale size (2/4/8), ignored for 0 or 1. */
    uint8_t base, scale; // 0x30
    /* Used when an operand is of type O_SMEM, O_MEM, O_DISP and the instruction has a displacement. This is the size of the 'disp' field in bytes. */
    uint8_t dispSize; // 0x32
    /* This is the offset of the 'disp' field in the instruction. */
    uint8_t dispOffset; // 0x33
    /* Meta defines the instruction set class, and the flow control flags. Use META macros. */
    uint8_t meta; // 0x34
    uint8_t reserved; // 0x35
    /* The CPU flags that the instruction operates upon. */
    uint16_t modifiedFlagsMask, testedFlagsMask, undefinedFlagsMask; // 0x36
    /* Reserved 4 bytes */ // 0x3C
} _DInst;
