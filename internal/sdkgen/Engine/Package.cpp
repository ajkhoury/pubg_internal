...
#include "UE4/PropertyFlags.h"
#include "UE4/FunctionFlags.h"
...
bool Package::Save(const std::experimental::filesystem::path& path) const
{
    ...
    if (generator->GetVirtualFunctionPatterns(c.FullName, patterns)) {
        ...
        for (auto&& pattern : patterns) {
            for (auto i = 0u; i < methodCount; ++i) {
                if (vtable[i] && utils::FindPattern((const void*)vtable[i], std::get<2>(pattern), 0xCC, reinterpret_cast<const unsigned char*>(std::get<0>(pattern)), std::get<1>(pattern))) {
...
void Package::SaveStructs(const std::experimental::filesystem::path& path) const
{
    extern IGenerator* generator;
...
void Package::SaveClasses(const std::experimental::filesystem::path& path) const
{
    extern IGenerator* generator;
...
void Package::SaveFunctions(const std::experimental::filesystem::path& path) const
{
    extern IGenerator* generator;
...
void Package::SaveFunctionParameters(const std::experimental::filesystem::path& path) const
{
    using namespace cpplinq;
