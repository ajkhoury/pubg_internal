...
)
{
    ...
    //
    if (ParentName && ResolvedNamespaceEntry->ValueCount > 1) {
...
)
{
    ...
    // Look for aliases in hosts libraries if necessary.
    //
    if (ParentName && ResolvedValueArray->Count > 1) {
...
)
{
    ...
    //
    // If the high index is less than the low index, then a matching namespace entry was
    // not found.
    //
    if (High < Low) {
        goto Exit;
...
ApiSetpSearchForApiSetHostV2(
    ...
    IN PCUNICODE_STRING ApiToResolve,
    IN PAPI_SET_NAMESPACE ApiSetNamespace
)
{
    LONG Low;
